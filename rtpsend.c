...
static Notify_value send_handler(Notify_client client)
{
  ...
  if (packet.length && send(sock[packet.type], packet.data, packet.length, 0) < 0) {
	  //modified by sujin for stop send while peer is closed.
	  //perror("write");
	  notify_stop();
	  exit(0);
	  return NOTIFY_DONE;
  ...
  if( packet.length > 0 )
  {
	  ...
	  int recvd = recv(sock[packet.type], buf, sizeof(buf), 0);
	  if (recvd > 0)
		  timeout_cnt = 0;
	  else {
		  timeout_cnt++;
...
int main(int argc, char *argv[])
{
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (sourceport) {
	  ...
	  if (setsockopt(sock[i], SOL_SOCKET, SO_RCVTIMEO, (const char*)&timeout, sizeof(timeout)) < 0) {
		  perror("SO_RCVTIMEO");
		  exit(1);
	  }
	  /*add end*/
